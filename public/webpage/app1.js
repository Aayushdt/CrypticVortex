...
// Cache DOM elements in one place for better performance and organization
const DOM = {
    // Views
    ...
    // Get all buttons for ripple effect
    getAllButtons() {
        return document.querySelectorAll('.btn, .btn-action, .btn-add, .people-indicator, button:not(.no-ripple)');
    }
};

// Validate DOM elements to prevent errors
const missingElements = Object.keys(DOM).filter(key => typeof DOM[key] !== 'function' && !DOM[key]);
...
const APP_STATE = {
    currentContainer: '',
    currentItemId: '',
    settledContainers: new Set(['cafeteria']), // Mark Cafeteria as already settled
    containerPeople: {
        'cafeteria': ['Ansh', 'Aarhan'],
        'shimla': ['Devansh', 'Aditya'],
        'dhabe': ['Ansh', 'Aarhan']
    },
    peopleTags: [], // For the "Start a Journey" modal
    selectedIcon: 'fas fa-utensils', // Default selected icon
    isInitialized: false,
    ...
    // Methods to update state safely
    ...
    addPeopleTag(personName) {
        if (!this.peopleTags.includes(personName)) {
            this.peopleTags.push(personName);
            return true;
        }
        return false;
    },
    
    removePeopleTag(personName) {
        this.peopleTags = this.peopleTags.filter(name => name !== personName);
    ...
    addPersonToContainer(containerId, personName) {
        if (!this.containerPeople[containerId]) {
            this.containerPeople[containerId] = [];
        }
        
        if (!this.containerPeople[containerId].includes(personName)) {
            this.containerPeople[containerId].push(personName);
            return true;
        }
        return false;
    },
    
    removePersonFromContainer(containerId, personName) {
        if (!this.containerPeople[containerId]) return false;
        
        this.containerPeople[containerId] = this.containerPeople[containerId]
            .filter(name => name !== personName);
        return true;
    },
    
    getPeopleInContainer(containerId) {
        return [...new Set(this.containerPeople[containerId] || [])];
    },
...
function addRippleEffect() {
    const buttons = DOM.getAllButtons();
    ...
    buttons.forEach(button => {
        ...
        button.addEventListener('click', function(e) {
            ...
            ripple.style.animationName = 'ripple-effect';
            ripple.style.animationDuration = '0.6s';
            ripple.style.animationTimingFunction = 'linear';
            ...
            // Calculate size (should be big enough to cover the button)
...
/**
 * Fade in an element with smooth animation
 * @param {HTMLElement} element - Element to fade in
 * @param {number} duration - Animation duration in ms
 * @returns {Promise} Resolves when animation completes
 */
function fadeIn(element, duration = APP_STATE.animations.duration) {
...
/**
 * Fade out an element with smooth animation
 * @param {HTMLElement} element - Element to fade out
 * @param {number} duration - Animation duration in ms
 * @returns {Promise} Resolves when animation completes
 */
function fadeOut(element, duration = APP_STATE.animations.duration) {
    return new Promise(resolve => {
        if (!element) {
            resolve();
            return;
...
/**
 * Slide in an element from the top
 * @param {HTMLElement} element - Element to slide in
 * @param {number} duration - Animation duration in ms
 * @returns {Promise} Resolves when animation completes
 */
function slideIn(element, duration = APP_STATE.animations.duration) {
    return new Promise(resolve => {
        if (!element) {
            resolve();
            return;
 ...
 * @param {number} scale - How much to scale (1.1 = 10% larger)
 * @param {number} duration - Animation duration in ms
 */
function bounceElement(element, scale = 1.05, duration = 300) {
    if (!element) return;
